...
trait LoanExportTrait {
    ...
    public function generateMemberExcel($members){
        return (new FastExcel($this->loanGenerator($members)))->download('MemberReport.xlsx',function($member){
            ...
            'Total Stock Penalt' =>$member->stock_dues->sum('penalty'),
            'Total Stock Penalt Paid' =>$member->stock_dues->sum('penalty_paid'),
            'Total Stock OutStanding' =>$member->stock_dues->sum('outstanding_amount'),
            'Total Fee Penalt' =>$member->fee_dues->sum('penalty'),
            'Total Fee Penalt Paid' =>$member->fee_dues->sum('penalty_paid'),
            'Total Fee OutStanding' =>$member->fee_dues->sum('outstanding_amount'),

...
class MemberSavingSummary extends Model
{
    ...
    'last_fee_amount','last_fee_purchase_date','stock_for_month','past_due_days','stock_penalty','financial_year_id'];
    ...
    public function dueStock(){
        return $this->hasMany(StockPastDue::class,'member_id','member_id')->where('financial_year_id',getFinancialYearId());
    }
    ...
    public function dueFees(){
        return $this->hasMany(FeePastDue::class,'member_id','member_id')->where('financial_year_id',getFinancialYearId());
    }
